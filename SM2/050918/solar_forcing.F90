...
integer :: io, ierr, nml_unit, stdlog_unit, t
...
if(add_solar_forcing) then
   if(solar_forcing_option == 'gaussian') then
      ...
      do t=1,ntime
        forcing(:,:,t) = solar_forcing1 * solar_forcing_mag1 * (1/solar_forcing_sf1) + &
      ...
      if(file_exist(trim(file))) then
         ...
         allocate (forcing_glb(ipts,jpts,12))
         ...
         if(mpp_pe() == mpp_root_pe()) print *, "size of forcing_glb", size(forcing_glb,1), size(forcing_glb,2), size(forcing_glb,3)
         ...
         do t=1,ntime
           call horiz_interp(Interp, forcing_glb(:,:,t), forcing(:,:,t))
         end do
         if(mpp_pe() == mpp_root_pe()) print *, "size of forcing", size(forcing,1), size(forcing,2), size(forcing,3)
